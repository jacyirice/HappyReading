...
const MeetingController = {
    ...
    uninscription: async(req, res) => {
        try {
            const meeting = await User_has_meeting.destroy({
                include: [{
                    model: Meeting,
                    where: {
                        id: req.params.id,
            ...
            if (!meeting) {
                return res.status(404).json({
                    ...
                    "errors": ["Você não esta inscrito no evento ou esse evento já ocorreu!"]
                });
            }
    ...
    store: async(req, res) => {
        ...
        try {
            let errors = []
            if (moment(req.body.datetime_start) <= moment().toDate())
                errors.push("meeting.datetime_start cannot be less than the current date");
            if (moment(req.body.datetime_start) >= moment(req.body.datetime_finish))
    ...
    update: async(req, res) => {
        ...
        try {
            const result = await sequelize.transaction(async(t) => {
                ...
                if (req.body.location && moment(meeting.datetime_start) > date_now) {
                    return res.status(400).json({
                ...
                if (req.body.location && updated_meeting.location) {
                    ...
                    console.log(emails, updated_meeting.location)
                    if (emails.length) {
                        const mailOptions = {
